...
public class ActivityInfo2Chain extends AbstractActivityChain {
    ...
    public Boolean action(ActionChainCheckAggregate actionChainCheckAggregate) {
        ...
        if (ActivityState.CREATED.equals(activityState)) {
            log.info("【充值】ACTIVITY_INFO 拦截（活动未开启）：activityId={}, activityState={}", activityEntity.getActivityId(), activityState);
            return false;
        }
        if (ActivityState.OVER.equals(activityState)) {
            log.info("【充值】ACTIVITY_INFO 拦截（活动已结束）：activityId={}, activityState={}", activityEntity.getActivityId(), activityState);
            return false;
        }

        LocalDateTime now = LocalDateTime.now();
        if (now.isBefore(activityEntity.getActivityBeginTime())) {
            log.info("【充值】ACTIVITY_INFO 拦截（活动未开启）：activityId={}, beginTime={}", activityEntity.getActivityId(), activityEntity.getActivityBeginTime());
            return false;
        }
        if (now.isAfter(activityEntity.getActivityEndTime())) {
            log.info("【充值】ACTIVITY_INFO 拦截（活动已结束）：activityId={}, endTime={}", activityEntity.getActivityId(), activityEntity.getActivityEndTime());
            return false;
        }
