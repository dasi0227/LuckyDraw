package com.dasi.types.constant;

public class RedisKey {

    public static final String STRATEGY_AWARD_KEY                   = "big_market:strategy:entity:strategy_award:";
    public static final String STRATEGY_KEY                         = "big_market:strategy:entity:strategy:";
    public static final String STRATEGY_RULE_KEY                    = "big_market:strategy:entity:strategy_rule:";
    public static final String STRATEGY_RULE_VALUE_KEY              = "big_market:strategy:entity:strategy_rule_value:";
    public static final String AWARD_KEY                            = "big_market:strategy:entity:award:";
    public static final String TREE_ID_KEY                          = "big_market:strategy:entity:tree_id:";
    public static final String RULE_TREE_VO_KEY                     = "big_market:strategy:entity:rule_tree_vo:";
    public static final String STRATEGY_ID_KEY                      = "big_market:strategy:entity:strategy_id:";
    public static final String AWARD_MAP_KEY                        = "big_market:strategy:entity:award_map:";
    public static final String RULE_LOCK_LIMIT_MAP_KEY              = "big_market:strategy:entity:rule_lock_limit_map:";
    public static final String STRATEGY_RATE_TABLE_KEY              = "big_market:strategy:rate:table:";
    public static final String STRATEGY_RATE_RANGE_KEY              = "big_market:strategy:rate:range:";
    public static final String STRATEGY_AWARD_STOCK_QUEUE_KEY       = "big_market:strategy:stock:queue:";
    public static final String STRATEGY_AWARD_STOCK_SURPLUS_KEY     = "big_market:strategy:stock:surplus:";
    public static final String ACTIVITY_KEY                         = "big_market:activity:entity:activity:";
    public static final String ACTIVITY_RECHARGE_SKU_KEY            = "big_market:activity:entity:activity_recharge_sku:";
    public static final String RECHARGE_QUOTA_KEY                   = "big_market:activity:entity:recharge_quota:";
    public static final String RECHARGE_SKU_STOCK_SURPLUS_KEY       = "big_market:activity:stock:recharge_sku_surplus:";
    public static final String RECHARGE_SKU_STOCK_QUEUE_KEY         = "big_market:activity:stock:recharge_sku_queue:";
    public static final String ACTIVITY_ID_KEY                      = "big_market:activity:entity:activity_id:";
    public static final String ACTIVITY_END_TIME_KEY                = "big_market:activity:entity:end_time:";
    public static final String BEHAVIOR_LIST                        = "big_market:behavior:entity:behavior_list:";
    public static final String PREFIX                               = "big_market:orderId:";
    public static final String RECHARGE_ORDER_ID_KEY                = "RECHARGE";
    public static final String RAFFLE_ORDER_ID_KEY                  = "RAFFLE";
    public static final String REWARD_ORDER_ID_KEY                  = "REWARD";
}
