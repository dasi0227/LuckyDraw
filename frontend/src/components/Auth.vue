...
const handleSubmit = async () => {
  ...
  try {
    if (mode.value === 'register') {
      ...
      if (reg?.token) {
        localStorage.setItem('luckydraw_token', reg.token);
      }
    }
    const loginRes = await api.login({ userId: form.userId, password: form.password });

    localStorage.setItem('luckydraw_user', form.userId);
    if (loginRes?.token) {
      localStorage.setItem('luckydraw_token', loginRes.token);
    }
    router.push({
      path: '/luckydraw/10001',
    });
  } catch (error) {
